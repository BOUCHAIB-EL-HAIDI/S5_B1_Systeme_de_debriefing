    ...
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 glass border-r border-white/10 p-6 flex flex-col fixed h-full">
            ...
            <nav class="space-y-2 flex-1">
                ...
                <a href="{{ $baseUrl }}/teacher/debriefing" class="flex items-center gap-3 p-3 rounded-xl text-white bg-indigo-500 shadow-lg shadow-indigo-500/20">
        ...
        <main class="flex-1 ml-64 p-8">
            ...
            <form action="{{ $baseUrl }}/teacher/debriefing/store" method="POST">
                                ...
                                <div>
                                    ...
                                    <select name="brief_id" required id="briefSelect" class="w-full bg-slate-900/50 border border-white/10 rounded-xl p-3 outline-none focus:border-indigo-500 transition-all text-slate-300">
                                        ...
                                        @foreach($briefs as $brief)
                                            <option value="{{ $brief['id'] }}">{{ $brief['title'] }} ({{ $brief['class_name'] }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
    ...
    <script>
        ...
        function checkLivrableStatus() {
            ...
            if (studentId && briefId) {
                livrableStatusEl.innerText = "Chargement...";
                fetch(`${baseUrl}/teacher/student/livrable?student_id=${studentId}&brief_id=${briefId}`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.status === 'Soumis ✓') {
                            let linkHtml = '';
                            if (data.content) {
                                linkHtml = ` <a href="${data.content}" target="_blank" class="underline ml-2 text-[10px] normal-case">Voir le travail</a>`;
                            }
                            
                            if (data.is_debriefed) {
                                livrableStatusEl.innerHTML = "Déjà évalué" + linkHtml;
                                livrableStatusEl.className = "px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-400 text-xs font-bold uppercase";
                                submitBtn.disabled = true;
                                submitBtn.classList.add('opacity-50', 'cursor-not-allowed');
                                submitBtn.title = "Ce brief a déjà été débriefé.";
                            } else {
                                livrableStatusEl.innerHTML = data.status + linkHtml;
                            ...
                            livrableStatusEl.innerText = "Non soumis";
                            livrableStatusEl.className = "px-3 py-1 rounded-full bg-rose-500/20 text-rose-400 text-xs font-bold uppercase";
                            submitBtn.disabled = true;
                            submitBtn.classList.add('opacity-50', 'cursor-not-allowed');
                            submitBtn.title = "L'étudiant doit rendre son travail avant d'être évalué.";
                        }
                    });
            } else {
        ...
        briefSelect.addEventListener('change', function() {
            ...
            currentBriefTitle.innerText = "Chargement...";

            fetch(`${baseUrl}/teacher/brief/competences?brief_id=${briefId}`)
                .then(res => res.json())
                .then(competences => {
