        ...
        <main class="flex-1 ml-64 p-8 flex justify-center">
            <div class="w-full max-w-3xl">
                <header class="mb-10 flex items-center gap-4">
                    <a href="{{ $baseUrl }}/teacher/briefs" class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all">
                ...
                <div class="glass p-8 rounded-[2.5rem] shadow-2xl">
                    <form action="{{ $baseUrl }}/teacher/briefs/store" method="POST" class="space-y-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <div>
                                    ...
                                    <select name="sprint_id" required class="w-full bg-slate-900/50 border border-white/10 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-slate-300">
                                        ...
                                        @foreach($sprints as $sprint)
                                            <option value="{{ $sprint['id'] }}">{{ $sprint['class_name'] }} - {{ $sprint['name'] }} (Ordre: {{ $sprint['order'] }})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-slate-900/50 border border-white/10 rounded-2xl">
                                    ...
                                    <label class="flex items-center gap-3 p-2 hover:bg-white/5 rounded-xl cursor-pointer transition-all">
                                        <input type="checkbox" name="competences[]" value="{{ $comp['code'] }}" class="w-4 h-4 rounded border-white/10 bg-slate-800 text-indigo-500 focus:ring-0">
                                        <span class="text-xs font-medium">{{ $comp['code'] }} - {{ $comp['label'] }}</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div>
